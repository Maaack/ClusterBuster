...
from .managers import ActiveLobbyManager
...
class Lobby(TimeStamped, SessionOptional):
    """
    Lobbies are for players and teams to join together.
    """
    ...
    active_lobbies = ActiveLobbyManager()
    ...
    def __setup_code(self):
        if not self.code:
            self.code = CodeGenerator.lobby_code()
    ...
    def has_player(self, player: Player) -> bool:
        """
        Returns `True` if the player is in the lobby.
        :param player: Player
        :return: bool
        """
        return self.players.filter(pk=player.pk).exists()
    ...
    def is_leader(self, player: Player) -> bool:
        """
        Returns `True` if the player is the lobby leader.
        :param player: Player
        :return: bool
        """
        return bool(self.session == player.session)
    ...
    def can_join(self, player: Player) -> bool:
        """
        Returns `True` if the player can join the lobby.
        :return: bool
        """
        return not self.has_player(player)
    ...
    def get_default_team(self) -> Team:
        """
        Returns the default team to join in the lobby.
    ...
    def join_team(self, player: Player, team: Team):
        """
        Attempts to join the player to a specific team in the lobby.
        ...
        if not self.has_team(team):
            raise Exception('Team does not exist in this Lobby.')
        team.join(player)

    def force_join(self, player: Player):
        """
        Forces the player into the lobby.
        :param player: Player
        :return: None
        """
        self.players.add(player)
    ...
    def join(self, player: Player, team=None):
        """
        Attempts to join the player to the lobby.
        Raises an exception if it fails.
        :param player: Player
        ...
        """
        if not self.can_join(player):
            raise Exception('Player can not join lobby.')
        self.force_join(player)
        if team is None:
...
class Activity(TimeStamped):
    """
    Activities give Lobbies a relative link to a game or some other activity.
    """

    lobby = models.ForeignKey(Lobby, on_delete=models.CASCADE, related_name='activities')
    name = models.CharField(_("Name"), max_length=64)
...
class ActivityOption(TimeStamped):
    """
    Activity Options give Activities prerequisites and a relative link that presumably starts a game or something.
    """
