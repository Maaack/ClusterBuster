from core.interfaces import Player2GameInterface, PartyInterface, RoomGamesInterface, Player2RoomInterface, Player2PartyRoundInterface, RoundInterface, PartyRoundInterface, GameRoundsInterface
...
class GameContext:
    ...
    def load(game):
        ...
        data = dict()
        data['game'] = game
        game_interface = GameRoundsInterface(game)
        game_is_started = game_interface.is_started()
        data['game_is_started'] = game_is_started
        if game_is_started:
            current_round = game_interface.get_current_round()
...
class RoundContext:
    ...
    def load(round_obj):
        ...
        round_interface = RoundInterface(round_obj)
        data['round_stage_name'] = round_interface.get_current_stage_name()
        data['round_is_composing'] = round_interface.is_composing()
        data['round_is_guessing'] = round_interface.is_guessing()
        data['round_is_done'] = round_interface.is_done()
        return data
...
class Player2GameContext:
    ...
    def load(player, game):
        ...
        """
        data = dict()
        player_2_game_interface = Player2GameInterface(player, game)
        ...
        party_2_round_data = Player2PartyRoundContent.load(player, player_party_round)
        data.update(party_2_round_data)
...
class Player2PartyRoundContent:
    @staticmethod
    def load(player, party_round):
        ...
        """
        data = dict()
        party_round_interface = PartyRoundInterface(party_round)
        data['party_round_is_waiting'] = party_round_interface.is_waiting()
        data['party_round_is_done'] = party_round_interface.is_done()
        data['party_round_guessing_players'] = party_round_interface.get_guessing_players()
        player_2_party_round_interface = Player2PartyRoundInterface(player, party_round)
...
class PartyRoundHintsContext:
    ...
    def load(party_round):
        ...
        :return: dict
        """
        target_words = party_round.target_words.order_by('order').all()
        data = [
            {'hint': target_word.get_hint_text(),
             'order': target_word.order + 1} for target_word in target_words]
        return data
