...
from core.models import Round, PartyRound, choices, TargetWord
...
class RoundInterface(interfaces.ModelInterface, interfaces.SetupInterface):
    ...
    def has_party_rounds(self):
        return self.round.party_rounds.count() == self.round.game.parties.count()
    ...
    def advance_if_all_teams_waiting(self):
        parties = self.round.game.parties
        all_parties_count = parties.count()
        waiting_parties_count = parties.filter(current_team_round__stage=choices.TeamRoundStages.WAITING.value).count()
        if all_parties_count == waiting_parties_count:
            self.round.party_rounds.update(stage=choices.TeamRoundStages.DONE.value)

    def advance_stage(self):
        parties = self.round.game.parties
        all_parties_count = parties.count()
        done_parties_count = parties.filter(current_team_round__stage=choices.TeamRoundStages.DONE.value).count()
...
class PartyRoundInterface(interfaces.ModelInterface, interfaces.SetupInterface):
    ...
    def get_current_stage_name(self):
        return choices.TeamRoundStages.choice(self.party_round.stage)

    def advance_stage(self):
        if self.party_round.stage in (choices.TeamRoundStages.ACTIVE.value, choices.TeamRoundStages.INACTIVE.value):
            self.party_round.stage = choices.TeamRoundStages.WAITING.value
            self.party_round.save()
        RoundInterface(self.party_round.round).advance_if_all_teams_waiting()

    def reset_stage(self):
        self.party_round.stage = choices.TeamRoundStages.ACTIVE.value
        self.party_round.save()

    def is_waiting(self):
        return self.party_round.stage == choices.TeamRoundStages.WAITING.value

    def is_active(self):
        return self.party_round.stage == choices.TeamRoundStages.ACTIVE.value

    def is_done(self):
        return self.party_round.stage == choices.TeamRoundStages.DONE.value

    def set_as_current_round(self):
        self.party_round.party.current_team_round = self
    ...
    def set_leader(self):
        ...
        self.party_round.leader = self.party_round.party.team.players.all()[offset]
